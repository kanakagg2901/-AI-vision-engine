...
// This file is kept exactly as the team wrote it (including the CHANGE 1/2/3
// optimization notes below, which were real, valid latency work). It is no
// longer the active model-runtime.js: Florence-2's autoregressive generate()
// call was still too slow for a live demo even after those optimizations
// (~multi-second per screen), so it was swapped for
// ../vision-engine/model-runtime.js (MediaPipe BlazeFace FaceDetector —
// single forward pass, real confidence scores, no generation step).
//
// If Florence-2 (or another generative grounding model) is wanted again
// later — e.g. to bring back generic "image"/photo-region detection, which
// BlazeFace doesn't do since it's face-only — this file is the starting
// point. Nothing here was thrown away.
// -----------------------------------------------------------------------
