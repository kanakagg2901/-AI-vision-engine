...
//
// NOTE: password-field is NOT detected by this file anymore, on purpose.
// If it's still missing from your final output, check that the DOM-based
// detector is actually running and its results are being merged back in
// downstream — this file will never produce it.
// -----------------------------------------------------------------------
const TARGET_LABELS = ["face", "image"]; // currently unused below — see note at bottom of file
...
export async function runInference(imageInput) {
  ...
  // Sanity check: CAPTION_TO_PHRASE_GROUNDING is expected to return
  // grounding.labels aligned with grounding.bboxes, but this isn't
  // guaranteed by every transformers.js version. If labels come back
  // missing, every box below silently defaults to "image" — log so that
  // failure mode is visible instead of silent.
  if (grounding && grounding.bboxes && !grounding.labels) {
    console.warn(
      '[detect.js] grounding.labels was not returned alongside grounding.bboxes — ' +
      'all boxes will default to "image". Verify processor.post_process_generation ' +
      'output shape for this transformers.js version.'
    );
  }
  ...
  if (grounding && grounding.bboxes) {
    for (let i = 0; i < grounding.bboxes.length; i++) {
      ...
      if (!isFullImageFallback) {
        ...
        // knowing the label in advance from the loop variable. We map
        // that returned phrase back to our exact contract label strings
        // here so labels stay exactly "face" or "image" as P4/P6 expect.
        // -------------------------------------------------------------
        const matchedText = (grounding.labels && grounding.labels[i])
          ? grounding.labels[i].toLowerCase()
          : "";
